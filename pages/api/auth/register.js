...
import { ensureString, signAccessToken, signRefreshToken, setAuthCookies, hashRefreshToken } from '@/lib/auth'
...
export default async function handler(req, res) {
	try {
		...
		if (!ensureString(email) || !ensureString(password, 200) || !ensureString(username, 32)) {
			return res.status(400).send({ error: 'register_bad_data' })
		}

		if (!ensureString(email) || !ensureString(password, 200) || password.length < 8 || !ensureString(username, 32)) {
			return res.status(400).send({ error: 'register_pwd_short' })
		}
		...
		if (existingEmail) return res.status(409).send({ error: 'register_email_taken' })

		const existingUsername = await User.findOne({ username: usernameNorm })
		if (existingUsername) return res.status(409).send({ error: 'register_username_taken' })
